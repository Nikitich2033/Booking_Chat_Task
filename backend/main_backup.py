...
class OllamaAI:
    ...
    def normalize_date_text(self, date_text: str) -> Optional[str]:
        ...
        Supports: YYYY-MM-DD, YYYY/MM/DD, DD-MM-YYYY, DD/MM/YYYY, 'today', 'tomorrow',
        'Month DD', 'next <weekday>', '<weekday>'.
        Returns ISO date string or None if cannot parse.
        """
        if not date_text:
            return None
        ...
        # Month name + day (assume current year, or next year if already passed)
        for fmt in ("%B %d", "%b %d"):
            try:
                ...
                pass
        # Weekday handling (optionally prefixed with 'next')
        weekdays = {
    ...
    async def extract_booking_intent(self, message: str) -> Optional[dict]:
        ...
        # Extract name (simple heuristic - capitalize words after "name is" or "I'm")
        name_patterns = [
            r'name is ([A-Za-z\s]+)',
            r'i\'m ([A-Za-z\s]+)',
            ...
            if intent.get('action') == 'check_availability':
                if intent.get('date') and intent.get('party_size'):
                    ...
                    if not normalized_date:
                        return "Please provide a date in YYYY-MM-DD format (e.g., 2025-08-12).", None, None, session_data
                    ...
                    if 'error' not in availability_result:
                        ...
                        # persist last availability search
                        ...
                        return f"Great! I found availability for {intent['party_size']} people on {date}. Here are the available time slots:", None, availability_data, session_data
                    else:
                        return f"I'm sorry, I couldn't check availability right now. {availability_result['error']}", None, None, session_data
                else:
                    return "To check availability, I need both the date and party size. Could you please provide both?", None, None, session_data
                ...
                if missing_fields:
                    # Store partial information in session
                    session_data.setdefault('booking', {})
                ...
                normalized_date = self.normalize_date_text(intent['date'])
                if not normalized_date:
                    return "Please provide the date in YYYY-MM-DD (e.g., 2025-08-12) so I can confirm availability.", None, None, session_data
                
                # Require availability confirmation for the slot
                ...
                if 'available_slots' in availability_check:
                    ...
                    if not slot_ok:
                        return (f"I couldn't verify availability for {date} at {intent['time']} for {intent['party_size']} people. "
                                f"Would you like me to show available times for that date?"), None, None, session_data
                ...
                if 'error' not in booking_result:
                    ...
                    # persist last confirmed booking
                    session_data.setdefault('booking', {})
                    session_data['booking'].update({
                        'reference': booking_result.get('booking_reference'),
                        'date': normalized_date,
                ...
                # Modify existing booking: need reference and at least one change
                ...
                # If date/time/party provided, verify availability for the new combo
                check_date = new_date or session_data.get('booking', {}).get('date')
                check_party = new_party or session_data.get('booking', {}).get('party_size')
                if check_date and check_party:
                    availability_check = await booking_client.check_availability(check_date, int(check_party))
                    if 'available_slots' in availability_check and new_time:
                        slot_ok = any(s.get('time','').startswith(new_time[:2]) and s.get('available') for s in availability_check['available_slots'])
                        if not slot_ok:
                            return (f"I couldn't verify availability for {check_date} at {new_time} for {check_party} people. "
                                    f"Would you like to pick from available times instead?"), None, None, session_data
 ...
 Keep tone kind and professional, but prioritize clarity and brevity."""
            ...
            print(f"Calling Ollama API: {self.base_url}/api/chat")
